...
class QosBandwidthLimitPatch(BaseModel):
    """
    The maximum QoS bandwidth limit for the volume, measured in `bytes per second`. Throttling occurs when throughput exceeds this limit. The maximum supported limit is `512 GB/s`. Specifying an empty string `\"\"` unsets the limit.
    """
    # data type: int
    ...
    def validate(cls, value):
        ...
        Pydantic calls this hook whenever QosBandwidthLimitPatch is used as a field type
        ...
        instead of the strict form:
            ...
            model.field = QosBandwidthLimitPatch("raw-string-value")

        This hook coerces all of those into a QosBandwidthLimitPatch so the
        `actual_instance` oneOf validator still runs against the wrapped value.
        """
        # Already a QosBandwidthLimitPatch: pass through untouched.
