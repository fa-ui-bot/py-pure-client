...
CONTAINERQOSBANDWIDTHLIMITPATCH_ONE_OF_SCHEMAS = ["int", "str"]

class ContainerQosBandwidthLimitPatch(BaseModel):
    """
    The QoS bandwidth floor for the container, measured in `bytes per second`. Priority is granted to traffic until this floor is reached. The maximum supported floor is `512 GB/s`. Specifying an empty string `\"\"` unsets the floor.
    """
    # data type: int
    ...
    if TYPE_CHECKING:
        ...
        actual_instance: Any
    one_of_schemas: List[str] = Field(CONTAINERQOSBANDWIDTHLIMITPATCH_ONE_OF_SCHEMAS, const=True)
    ...
    def validate(cls, value):
        ...
        Pydantic calls this hook whenever ContainerQosBandwidthLimitPatch is used as a field type
        ...
        instead of the strict form:
            ...
            model.field = ContainerQosBandwidthLimitPatch("raw-string-value")

        This hook coerces all of those into a ContainerQosBandwidthLimitPatch so the
        `actual_instance` oneOf validator still runs against the wrapped value.
        """
        # Already a ContainerQosBandwidthLimitPatch: pass through untouched.
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ContainerQosBandwidthLimitPatch.construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ContainerQosBandwidthLimitPatch with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ContainerQosBandwidthLimitPatch with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> ContainerQosBandwidthLimitPatch:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> ContainerQosBandwidthLimitPatch:
        """Returns the object represented by the json string"""
        return ContainerQosBandwidthLimitPatch.construct(_fields_set=None, **{
