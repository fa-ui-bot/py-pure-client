...
class ActiveDirectoryPost(BaseModel):
    ...
    ActiveDirectoryPost
    """
    ca_certificate: Optional[Reference] = Field(default=None, description="The reference (`ID` and `name`) of the Certificate Authority (CA) that signed the certificates of the configured servers, which is used to validate the authenticity of the servers.")
    ca_certificate_group: Optional[Reference] = Field(default=None, description="The reference (`ID` and `name`) of a certificate group containing CA certificates that can be used to validate the authenticity of the configured servers.")
    ...
    sources: Optional[conlist(Reference)] = Field(default=None, description="A list of network interfaces used for egress. For directory services at the array level, this list can be empty. If the list is empty, the system will use VIFs with the `ds` service (or if none, the `management` service) for egress.")
    tls: Optional[StrictStr] = Field(default=None, description="The TLS mode for communication with domain controllers. Values include `required` and `optional`. `required` forces TLS communication with a domain controller. `optional` allows the use of non-TLS communication; TLS will still be preferred, if available. If not specified, defaults to `required`.")
    ...
    __properties = ["ca_certificate", "ca_certificate_group", "computer_name", "directory_servers", "domain", "join_ou", "kerberos_servers", "password", "sources", "tls", "user"]
