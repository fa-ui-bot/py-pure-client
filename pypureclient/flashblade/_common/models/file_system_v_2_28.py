...
class FileSystem(BaseModel):
    ...
    promotion_status: Optional[StrictStr] = Field(default=None, description="Possible values are `promoted`, `demoted`, `demoting` and `promoting`. Indicates the current promotion state of the file system with respect to replication and file system copy. The state can change based on updates to requested_promotion_state or when running a planned failover using quiesce. New file systems start in the promoted state. For file system copy operations, the state is set to promoting until the copy completes. During a planned failover, the file system moves to demoting while data replication is in progress, and transitions to demoted once replication is complete.")
