...
class NetworkInterfacePatch(BaseModel):
    ...
    rdma_enabled: Optional[StrictBool] = Field(default=None, description="If `true` indicated that RDMA is enabled on the network interface. If `false`, RDMA is disabled on the network interface. The RDMA feature is only supported on `data` service type of the interface. Specifying `true` when `services` contains values other than `data` will result in an error.")
    ...
    __properties = ["address", "attached_servers", "rdma_enabled", "services"]
